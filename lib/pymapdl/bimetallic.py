...
def my_mapdl_launch_in_cwd(my_wdirnew, my_job_name):
    ...
    mapdl = launch_mapdl(run_location=new_wdir_path , jobname = my_job_name, 
                      nproc = 1 ,override = True, additional_switches='-smp')
    return mapdl,new_wdir_path


def solve_vm_35(images,mapdl,length,thickness,ex_mat1,ex_mat2,cte_mat1,cte_mat2,my_t_ref,my_t_amb):
    ...
    sbar_kwargs = {"color": "black", "title": "Z Displacement (inch)", "vertical": False, "n_labels": 9}
    mapdl.post_processing.plot_nodal_displacement('Z', '', cpos='iso', background='white',
                                                  edge_color='black', show_edges=True, scalar_bar_args=sbar_kwargs,
                                                  n_colors=9, savefig=png_path)
...
def roarks_vm_35(length,thickness,ex_mat1,ex_mat2,cte_mat1,cte_mat2,my_t_ref,my_t_amb):
    ...
    K1 = 4. + 6. + 4. + (ex_mat1/ex_mat2) + (ex_mat2/ex_mat1)
    ymax = (6. * (cte_mat2 - cte_mat1) * (my_t_amb - my_t_ref) * thickness) / (K1 * ((thickness/2)**2))
    ymax = (ymax * (length ** 2)) / 2.
    return round(ymax, 3)
